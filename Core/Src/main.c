...
void ExponentialFilter(MPU6050_Data *sensorValue)
{
    ...
    filteredSensorData.Accel_X =  filteredSensorData.Accel_X;

    filteredSensorData.Accel_Y =  filteredSensorData.Accel_Y;

    filteredSensorData.Accel_Z =  filteredSensorData.Accel_Z;
...
int main(void)
{
  ...
  while (1)
  {
      ...
      // Step 3: Set the timers/PWM connected to the motors
...
static void MX_TIM1_Init(void)
{
  ...
  htim1.Init.Period = 19999;                           // 50Hz PWM frequency; good enough for the motors
